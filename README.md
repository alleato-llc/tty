...
- **detach a pane-tab**: right-click → **Detach tab** — it opens in its own window and, on
  reattach, returns to its original tab group (or docks onto the main strip as a new tab if
  that group is gone)
...
- **Env view (`⌘⇧E`)** — a movable, resizable popover (like the metric charts, so it
  ...
  opens **compact** — a masked list (a key/value column that stays compact and scrolls
  when there are many) plus an Add button — and the expand control (`+` / `−`, the same
  glyph cluster the metric drill-ins use) reveals the full experience: a filter, the
  **Reveal values** toggle, and the source note. Click a row to copy `NAME=value`. Open it with `⌘⇧E`, the pane menu, or an **Env** status-bar cell (add
...
like JetBrains Mono / Fira Code). A live **preview** on the Theme tab renders a sample
terminal in the chosen theme, font, size, and ligatures — updating as you change them.
Elsewhere in Appearance, toggle **Highlight active tab** (accent ink vs. a subtler emphasis) and
